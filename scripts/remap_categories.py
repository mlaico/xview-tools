...
import h4dlib.data.image_util as img_ops
...
def generate_images_and_annotations(
    ...
    for img_id in tqdm([list(coco.imgs.keys())[0]]):
        img = coco.loadImgs(img_id)[0]
        I = io.imread("%s/%s" % (source_imgs_dir, img["file_name"]))

        img_ops.vis_coco_anns(I, coco, [img], vis_out)

        chip_name = img["id"]
        coords = all_coords[all_chips == chip_name]
        classes = all_classes[all_chips == chip_name].astype(np.int64)
        chips, chip_boxes, chip_classes = img_ops.chip_image_w_overlap(I, coords, classes)
...
def main(datatype):
    ...
    DEST_JSON = DATADIR / "Xview/coco_chipped_overlap_visual/{}_{}_chipped.json".format(
        h4dconfig.XVIEW_COCO_PREFIX, datatype
    )
    SOURCE_IMGS_DIR = DATADIR / "Xview/coco_vanilla"
    DEST_IMGS_DIR = DATADIR / "Xview/coco_chipped_overlap_visual/{}".format(datatype)
    VIS_OUT = DATADIR / "Xview/coco_chipped_overlap_visual/vis/{}".format(datatype)
    IMG_TAG = datatype
