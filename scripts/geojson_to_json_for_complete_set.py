...
def generate_images_and_annotations(class_ids, source_imgs_dir, dest_imgs_dir, img_tag, xview_json, catLookup):
	...
	all_imgs = np.array(["U-LIMDIS_" + img for img in all_imgs])
	...
	# print("PRINTING EXISTING IMAGES... \n")
	# print(existing_imgs)
	# print("\n")
	...
	for image in tqdm(all_imgs):
		...
		file_index = int(Path(image).stem.split("_")[-1])
		if not os.path.exists(dest_imgs_dir):
			os.makedirs(dest_imgs_dir)	
		image_dict = {}
		image_dict['license'] = 1
		image_dict['file_name'] = 'XVIEW_' +  img_tag + '_' + str(file_index).zfill(12) + ".jpeg"
		image_dict['width'] = width
		image_dict['height'] = height
		image_dict['id'] = file_index
		images.append(image_dict)
		new_image = convertToJpeg(arr)
		with open(os.path.join(dest_imgs_dir,'XVIEW_' +  img_tag + '_' + str(file_index).zfill(12) + ".jpeg"), 'wb') as image_file:
...
def generate_categories(class_labels_path):

	categories = []
	class_ids = []
	# class_labels_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../xview_class_labels_complete.txt")
	with open(class_labels_path) as class_labels_file:
		while True:
			...
			name = name.split('\n')[0]
			class_entry = {}
			class_entry['supercategory'] = "None"
...
def main():
	...
	proj_root = "/home/laielli/data"
	DATA_DIR = os.path.join(proj_root,"Xview")
	LIMDIS_DIR = os.path.join(DATA_DIR,"LIMDIS")
	CLASS_TXT = os.path.join(DATA_DIR, "xview_class_labels_complete.txt")
	XVIEW_JSON = os.path.join(LIMDIS_DIR, "U-LIMDIS_xView_Final.geojson")
	IMG_TAG = "2017"
	SOURCE_IMGS_DIR = os.path.join(LIMDIS_DIR, "images")
	DEST_IMGS_DIR = os.path.join(DATA_DIR, "coco_limdis")
	DEST_JSON = os.path.join(DEST_IMGS_DIR, "xview_coco_limdis_v0.json")
	...
	with open(DEST_JSON, 'w') as coco_file:
		root_json = {}
		categories_json, class_ids = generate_categories(CLASS_TXT)
